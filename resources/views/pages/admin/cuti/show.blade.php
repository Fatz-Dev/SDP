...
@section('content')
    <div class="page-header d-flex align-items-center justify-content-between">
        <div>
            ...
            <p class="text-muted">Informasi lengkap permohonan cuti pegawai.</p>
        </div>
        <a href="{{ route('admin.cuti.index') }}" class="btn btn-light rounded-pill px-4">
        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                ...
                <div class="card-body p-4">
                    <div class="row g-4">
                        <div class="col-md-6">
                            <label class="text-muted small fw-bold text-uppercase d-block mb-1">Nama Pegawai</label>
                            <div class="fw-bold fs-5 text-dark">{{ $cuti->user->name }}</div>
                            <small class="text-muted">{{ $cuti->user->jabatan ?? 'Jabatan tidak diatur' }}</small>
                        </div>
                        <div class="col-md-6 text-md-end">
                            <label class="text-muted small fw-bold text-uppercase d-block mb-1">Status Saat Ini</label>
                            @php
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="fw-bold text-dark">{{ $cuti->jenis_cuti }}</div>
                        </div>
                        <div class="col-md-6">
                        ...
                        @if ($cuti->catatan_admin)
                            <div class="col-12">
                                <label class="text-muted small fw-bold text-uppercase d-block mb-1">Catatan Admin</label>
                                <div
        ...
        <div class="col-lg-4">
            @if ($cuti->status == 'Pending')
                <div class="card border-0 shadow-sm rounded-4 position-sticky" style="top: 2rem;">
                    <div class="card-header bg-white border-0 py-4 px-4">
                        <h5 class="fw-bold mb-0"><i class="fas fa-gavel me-2 text-primary"></i> Keputusan Admin</h5>
                    </div>
                    <div class="card-body p-4">
                        <form action="{{ route('admin.cuti.status', $cuti->id) }}" method="POST">
                            @csrf
                            @method('PATCH')
                            ...
                            <div class="mb-3">
                                ...
                                <div class="btn-group w-100" role="group" aria-label="Status selection">
                                    <input type="radio" class="btn-check" name="status" id="status_approve"
                                        value="Disetujui" required checked>
                                    <label class="btn btn-outline-success py-2" for="status_approve">
                                        <i class="fas fa-check me-1"></i> Setujui
                                    </label>

                                    <input type="radio" class="btn-check" name="status" id="status_reject"
                                        value="Ditolak" required>
                                    <label class="btn btn-outline-danger py-2" for="status_reject">
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="catatan_admin" class="form-control rounded-3" rows="4"
                                    placeholder="Berikan alasan atau keterangan tambahan..."></textarea>
                            </div>
            ...
            @else
                <div class="card border-0 shadow-sm rounded-4">
                    <div class="card-body p-4 text-center">
                        ...
                        <p class="text-muted small">Cuti ini telah diproses dan statusnya adalah
                            <strong>{{ $cuti->status }}</strong>.</p>
                        <hr>
                        <p class="small text-muted mb-0">Tidak dapat mengubah keputusan yang sudah dibuat.</p>
                    </div>
                </div>
            @endif
        </div>
